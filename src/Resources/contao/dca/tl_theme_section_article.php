 ...
 */
$GLOBALS['TL_DCA']['tl_theme_section_article'] = array
(
...
//            array('tl_theme_section_article', 'addCustomLayoutSectionReferences'),
    ...
    (
        ...
        (
            ...
            'paste_button_callback'   => array('tl_theme_section_article', 'pasteArticle'),
        ...
        (
            ...
            'label_callback'          => array('tl_theme_section_article', 'addIcon')
        ),
...
//                'button_callback'     => array('tl_theme', 'editCss')
        ...
        (
            ...
            (
                ...
                'icon'                => 'edit.svg',
                'button_callback'     => array('tl_theme_section_article', 'editArticle')
            ),
            'editheader' => array
            (
                ...
                'href'                => 'act=edit',
                'icon'                => 'header.svg',
                'button_callback'     => array('tl_theme_section_article', 'editHeader')
            ),
            'copy' => array
            (
                ...
                'button_callback'     => array('tl_theme_section_article', 'copyArticle')
            ),
            'cut' => array
            (
                ...
                'icon'                => 'cut.svg',
                'attributes'          => 'onclick="Backend.getScrollOffset()"',
                'button_callback'     => array('tl_theme_section_article', 'cutArticle')
            ),
            'delete' => array
            (
                ...
                'button_callback'     => array('tl_theme_section_article', 'deleteArticle')
            ),
            'toggle' => array
            (
                'label'               => &$GLOBALS['TL_LANG']['tl_theme_section_article']['toggle'],
                'icon'                => 'visible.svg',
                'attributes'          => 'onclick="Backend.getScrollOffset();return AjaxRequest.toggleVisibility(this,%s)"',
                'button_callback'     => array('tl_theme_section_article', 'toggleIcon')
            ),
            'show' => array
            (
    ...
    (
        ...
        (
            array('tl_theme_section_article', 'addAliasButton')
        )
    ),
    ...
    (
        ...
        (
            ...
            'save_callback' => array
            (
                array('tl_theme_section_article', 'generateAlias')
            ),
        ...
        (
            ...
            'default'                 => 'main',
            'inputType'               => 'select',
            'options_callback'        => array('tl_theme_section_article', 'getActiveLayoutSections'),
        ...
        (
            ...
            'exclude'                 => true,
            'inputType'               => 'select',
            'options_callback'        => array('tl_theme_section_article', 'getArticleTemplates'),
 ...
 */
class tl_theme_section_article extends Backend
{
    ...
    public function checkPermission()
    {
        ...
        if (\is_array($session['CURRENT']['IDS']))
        {
            ...
            {
                $objArticle = $this->Database->prepare("SELECT p.pid, p.includeChmod, p.chmod, p.cuser, p.cgroup FROM tl_theme_section_article a, tl_page p WHERE a.id=? AND a.pid=p.id")
                    ->limit(1)
                    ->execute($id);
        ...
        if (isset($session['CLIPBOARD']['tl_theme_section_article']) && \is_array($session['CLIPBOARD']['tl_theme_section_article']['id']))
        {
            $clipboard = array();

            foreach ($session['CLIPBOARD']['tl_theme_section_article']['id'] as $id)
            {
                $objArticle = $this->Database->prepare("SELECT p.pid, p.includeChmod, p.chmod, p.cuser, p.cgroup FROM tl_theme_section_article a, tl_page p WHERE a.id=? AND a.pid=p.id")
                    ->limit(1)
                    ->execute($id);
            ...
            $session['CLIPBOARD']['tl_theme_section_article']['id'] = $clipboard;
        }
        ...
        if (Input::get('act') && Input::get('act') != 'paste')
        {
            ...
            $objPage = $this->Database->prepare("SELECT pid FROM tl_theme_section_article WHERE id=?")
                ->limit(1)
            ...
            switch (Input::get('act'))
            {
                ...
                case 'cutAll':
                    ...
                    {
                        $objParent = $this->Database->prepare("SELECT id, type FROM tl_page WHERE id=(SELECT pid FROM tl_theme_section_article WHERE id=?)")
                            ->limit(1)
                            ->execute(Input::get('pid'));
    ...
    {
        if (!$this->User->canEditFieldsOf('tl_theme_section_article'))
        {
    ...
    public function toggleIcon($row, $href, $label, $title, $icon, $attributes)
    {
        ...
        if (!$this->User->hasAccess('tl_theme_section_article::published', 'alexf'))
        {
            return '';
        }
    ...
    public function toggleVisibility($intId, $blnVisible, DataContainer $dc=null)
    {
        ...
        if (\is_array($GLOBALS['TL_DCA']['tl_theme_section_article']['config']['onload_callback']))
        {
            foreach ($GLOBALS['TL_DCA']['tl_theme_section_article']['config']['onload_callback'] as $callback)
            {
        ...
        if ($dc)
        {
            $objRow = $this->Database->prepare("SELECT * FROM tl_theme_section_article WHERE id=?")
                ->limit(1)
                ->execute($intId);
        ...
        $objVersions = new Versions('tl_theme_section_article', $intId);
        $objVersions->initialize();

        // Trigger the save_callback
        if (\is_array($GLOBALS['TL_DCA']['tl_theme_section_article']['fields']['published']['save_callback']))
        {
            foreach ($GLOBALS['TL_DCA']['tl_theme_section_article']['fields']['published']['save_callback'] as $callback)
            {
                if (\is_array($callback))
                {
                    $this->import($callback[0]);
        ...
        // Update the database
        $this->Database->prepare("UPDATE tl_theme_section_article SET tstamp=$time, published='" . ($blnVisible ? '1' : '') . "' WHERE id=?")
            ->execute($intId);
        ...
        if (\is_array($GLOBALS['TL_DCA']['tl_theme_section_article']['config']['onsubmit_callback']))
        {
            foreach ($GLOBALS['TL_DCA']['tl_theme_section_article']['config']['onsubmit_callback'] as $callback)
            {
                if (\is_array($callback))
                {
                    $this->import($callback[0]);
